...
// also increase position if so needed...
  ...
  t1 = n2.time-c;
  t = n2.time-n1.time;
  partModel.boxX = Math.round((t1*n1.x + t2*n2.x)/t);
  partModel.yOffset = Math.round((t1*n1.yoff + t2*n2.yoff)/t);
...
function drawImage(ctx){
  ...
  ctx.drawImage(image,
    ...
    partModel.yOffset,
    window.innerWidth,
    Math.round(window.innerWidth*imagesize.altus.height/imagesize.altus.width)
  );
}
...
partSelect.change(function(event){
  ...
  image.src= "pieces/zirlerMotet/images/"+(partSelect.val());
  ctx = thePartCanvas.getContext("2d");
  image = document.getElementById("source");
...
initPartButton.click(function(event){
